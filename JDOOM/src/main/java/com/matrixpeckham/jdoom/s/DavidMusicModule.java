...
//TODO: this and the MUS loader were ripped from mocha doom,
//and I'm not sure I want to keep them
/**
 * RIPPED FROM MOCHA DOOM SOURCE
 * <p>
...
public class DavidMusicModule {
    ...
    private static DavidMusicModule inst = null;

    public static DavidMusicModule getInstance() {
        if (inst == null) {
            inst = new DavidMusicModule();
        }
        return inst;
    }
    ...
    public void InitMusic() {
        try {
            ...
            sendSysexMessage(receiver, (byte) 0xf0, (byte) 0x7e, (byte) 0x7f,
                    (byte) 9, (byte) 1, (byte) 0xf7);
    ...
    private static void sendControlChange(Receiver receiver, int midiChan,
            int ctrlId, int value) {
    ...
    public void setSong(Song s) {
        try {
            sequencer.stop();
            sequencer.setSequence(s.sequence);
            songloaded = true;
        } catch (Exception e) {
            throw new IllegalArgumentException("Problem with song", e);
        }
    }
    ...
    private void setPitchBendSensitivity(Receiver receiver, int midiChan,
            int semitones) {
        sendRegParamChange(receiver, midiChan, 0, 0, 2);
    }

    private void sendRegParamChange(Receiver receiver, int midiChan,
            int paramMsb, int paramLsb, int valMsb) {
