...
public class VolumeScalingReceiver implements Receiver {
    ...
    public static VolumeScalingReceiver getInstance() {
        try {
            List<MidiDevice.Info> dInfos
                    = new ArrayList<MidiDevice.Info>(Arrays.asList(MidiSystem.
                            getMidiDeviceInfo()));
    ...
    /** A collection of kludges to pick a synthesizer until cvars are
     * implemented */
                    ...
                    if (dev instanceof Sequencer) {
                        ...
                        if (dev instanceof Synthesizer) {
                            ...
                            if (lcName.contains("java")) {
                                ...
                                if (((Synthesizer) dev).getDefaultSoundbank()
                                        != null) {
                                    result -= 10;
                                } else {
    ...
    private int getVolumeChangeChannel(MidiMessage message) {
        if (message.getLength() >= 3) {
            ...
            if ((byte) 0xb0 <= mBytes[0] && mBytes[0] < (byte) 0xc0 && mBytes[1]
                    == 7) {
                return mBytes[0] & 15;
            }
        }
