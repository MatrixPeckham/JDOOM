...
import static com.matrixpeckham.jdoom.util.Fixed.FRAC_UNIT;
...
public class Tables {
     ...
     *
     * However, some problems may arise with comparisons and ordinary
     * arithmetic:
     ...
     * with 0x100000000 or else long ints will store angles exceeding 360
     * degrees!
     * Under no circumstances the value actually stored in the "long angles"
     * should
     * exceed 0xFFFFFFFF.
     *
     * An example: comparing any two long angles directly is OK, provided they
     * were
     * constructed correctly.
     *
     * Adding, subtracting, multiplying etc. with two or more angles however
     * requires
     * rollover compensation (e.g. result=(a+b+c) is wrong,
    ...
    public static final int DBITS = FRAC_BITS - SLOPEBITS;
...
// Range goes from 0x00000000 to 0x20000000, so in theory plain ints should be enough...
    /** This maps a value 0-2048 to a BAM unsigned integer angle, ranging from
     * 0x0 to 0x2000000:
     * <p>
     * In practice, this means there are only tangent values for angles up to 45
     * degrees.
     * <p>
    ...
    /** Use this to get a value from the finesine table. It will be
     * automatically shifte,
    ...
    /** Use this to get a value from the finecosine table. It will be
    ...
    /** Converts a long angle to a BAM LUT-ready angle (13 bits, between
     * 0-8191).
     * Cuts away rollover.
     *
     * @param angle
    ...
    /** Converts a long angle to a TAN BAM LUT-ready angle (12 bits, between
     * 0-4195).
     * Cuts away rollover.
     *
     * @param angle
    ...
    /** Converts an 32-bit int angle to a BAM LUT-ready angle (13 bits, between
     * 0-8192).
     *
     * @param angle
     *
     ...
     * found to, indeed, produce the same values found in the
     * finesine/finecosine
    ...
    public static void InitTables() {
        ...
        /* tantoangle table
         * There was actually no dead code for that one, so this is a close
         * recreation.
         * Since there are 2049 values, and the maximum angle considered is
         * 536870912 (0x20000000)
         * which is 45 degrees in BAM, we have to fill in the atan for values up
         * to 45 degrees.
         * Since the argument is a slope ranging from 0...2048, we have 2049
         * equally spaced (?)
         * values, with 2048 being being the unitary slope (0x10000 in fixed_t).
         * That value is only
         * accessed in special cases (overflow) so we only need to consider
         * 0..2047 aka 11 bits.
         * So: we take "minislopes" 0-2048, we blow them up to a full fixed_t
         * unit with <<5.
         * We make this into a float (?), then use trigonometric ATAN, and then
         * go to BAM.
         *
         * Any questions?
         *
